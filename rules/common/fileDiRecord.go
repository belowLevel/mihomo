...
import (
	...
	D "github.com/metacubex/mihomo/rules/domain"
)

var (
	setsRecord    map[string]*D.DomainSet = make(map[string]*D.DomainSet)
	fdrdoonce     sync.Once
...
func matchISO(ipAddress net.IP, host string, domainSet *D.DomainSet, metadata *C.Metadata) bool {
	if !metadata.GeoedIp() {
